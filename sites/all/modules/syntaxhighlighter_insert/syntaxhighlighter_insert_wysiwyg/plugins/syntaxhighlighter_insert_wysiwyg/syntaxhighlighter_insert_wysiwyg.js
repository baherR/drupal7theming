...
Drupal.wysiwyg.plugins.syntaxhighlighter_insert_wysiwyg = {
  ...
  insert_form: function (data, settings, instanceId) {
    ...
    var aurl = '';
    if (Drupal.settings.syntaxhighlighter_insert_wysiwyg.clean_url) {
      aurl = Drupal.settings.basePath + 'syntaxhighlighter_insert_wysiwyg/insert/' + form_id;
    }
    else {
      aurl = Drupal.settings.basePath + 'index.php?q=syntaxhighlighter_insert_wysiwyg/insert/' + form_id;
    }
    ...
    dialogdiv.load(aurl + " #syntaxhighlighter-insert-wysiwyg-form", function(){
      ...
      btns[Drupal.t('Insert syntaxhighlighter tag')] = function () {
        ...
        var code = dialogdiv.contents().find('#' + field_id.replace('*field*', 'texte-code')).val();
